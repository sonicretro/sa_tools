...
{
    public partial class TextureListEditor : Form
    {
        ...
        private List<KeyValuePair<string, TextureListEntry[]>> textureLists = new List<KeyValuePair<string, TextureListEntry[]>>();

        private TextureListEntry[] CurrentList
        {
            get { return textureLists[levelList.SelectedIndex].Value; }
            set
            {
                textureLists[levelList.SelectedIndex] = new KeyValuePair<string, TextureListEntry[]>(textureLists[levelList.SelectedIndex].Key, value);
            }
        }
        ...
        private void TextureListEditor_Load(object sender, EventArgs e)
        {
            ...
            foreach (KeyValuePair<string, FileInfo> item in Program.IniData.Files)
                if (item.Value.Type.Equals("texlist", StringComparison.OrdinalIgnoreCase))
                {
                    textureLists.Add(new KeyValuePair<string, TextureListEntry[]>(item.Value.Filename, TextureList.Load(item.Value.Filename)));
        ...
        private void TextureListEditor_FormClosing(object sender, FormClosingEventArgs e)
        {
            switch (MessageBox.Show(this, "Do you want to save?", Text, MessageBoxButtons.YesNoCancel, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1))
            {
                ...
                case DialogResult.Yes:
                    foreach (KeyValuePair<string, TextureListEntry[]> item in textureLists)

...
{
    public partial class TrialLevelListEditor : Form
    {
        public TrialLevelListEditor()
        {
            InitializeComponent();
        }
        ...
        private void TrialLevelListEditor_Load(object sender, EventArgs e)
        {
            levelList.BeginUpdate();
            foreach (KeyValuePair<string, FileInfo> item in Program.IniData.Files)
                if (item.Value.Type.Equals("triallevellist", StringComparison.OrdinalIgnoreCase))
                {
                    levelLists.Add(new KeyValuePair<string, SA1LevelAct[]>(item.Value.Filename, TrialLevelList.Load(item.Value.Filename)));
        ...
        private void TrialLevelListEditor_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void ReloadObjectList()
        {
            ...
            addButton.Enabled = CurrentList.Length < 10;
            ReloadObjectData();
        }
        ...
        private void addButton_Click(object sender, EventArgs e)
        {
            ...
            addButton.Enabled = currentList.Count < 10;
        }
